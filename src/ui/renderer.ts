...
export function header(workspace: string, model: string, permission: string): void {
  process.stdout.write(`${chalk.bold('OpenSyntax')} ${chalk.gray('|')} ${chalk.cyan(model)} ${chalk.gray('|')} ${permission} ${chalk.gray('|')} ${workspace}\n`);
  process.stdout.write(chalk.gray('Type /help for commands. Ctrl+C or /exit to quit.\n\n'));
}
